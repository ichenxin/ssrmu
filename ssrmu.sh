...
BBR_installation_status(){
	if [[ ! -e ${BBR_file} ]]; then
		...
		cd "${file}"
		if ! wget -N --no-check-certificate https://raw.githubusercontent.com/ichenxin/doubi/master/bbr.sh; then
...
ss_link_qr(){
	...
	SSurl="ss://${SSbase64}"
	SSQRcode="https://cli.im/api/qrcode/code?text=${SSurl}&mhid=5EeTCQy7zp0hMHcpK91VOK8"
...
ssr_link_qr(){
	...
	SSRQRcode="https://cli.im/api/qrcode/code?text=${SSRurl}&mhid=5EeTCQy7zp0hMHcpK91VOK8"
...
Set_config_user(){
	...
	read -e -p "(默认: ssr):" ssr_user
	[[ -z "${ssr_user}" ]] && ssr_user="ssr"
...
Set_config_port(){
	...
	read -e -p "(默认: 3000):" ssr_port
	[[ -z "$ssr_port" ]] && ssr_port="3000"
...
Set_config_password(){
	echo "请输入要设置的用户 密码"
	read -e -p "(默认: 6B1nt08HTu1):" ssr_password
	[[ -z "${ssr_password}" ]] && ssr_password="6B1nt08HTu1"
...
Set_config_method(){
 ...
 ${Tip} salsa20/chacha20-*系列加密方式，需要额外安装依赖 libsodium ，否则会无法启动ShadowsocksR !" && echo
	read -e -p "(默认: 16. chacha20-ietf):" ssr_method
...
Download_SSR(){
	cd "/usr/local"
	wget -N --no-check-certificate "https://github.com/ichenxin/shadowsocksr/archive/manyuser.zip"
	#git config --global http.sslVerify false
	#env GIT_SSL_NO_VERIFY=true git clone -b manyuser https://github.com/ichenxin/shadowsocksr.git
...
Service_SSR(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate https://raw.githubusercontent.com/ichenxin/doubi/master/service/ssrmu_centos -O /etc/init.d/ssrmu; then
		...
		if ! wget --no-check-certificate https://raw.githubusercontent.com/ichenxin/doubi/master/service/ssrmu_debian -O /etc/init.d/ssrmu; then
			echo -e "${Error} ShadowsocksR服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/ssrmu
...
View_user_connection_info(){
 ...
 ${Green_font_prefix}2.${Font_color_suffix} 显示 IP+IP归属地 格式" && echo
	read -e -p "(默认: 2):" ssr_connection_info
	[[ -z "${ssr_connection_info}" ]] && ssr_connection_info="2"
...
# 封禁 BT PT SPAM
BanBTPTSPAM(){
	wget -N --no-check-certificate https://raw.githubusercontent.com/ichenxin/doubi/master/ban_iptables.sh && chmod +x ban_iptables.sh && bash ban_iptables.sh banall
	rm -rf ban_iptables.sh
}
# 解封 BT PT SPAM
UnBanBTPTSPAM(){
	wget -N --no-check-certificate https://raw.githubusercontent.com/ichenxin/doubi/master/ban_iptables.sh && chmod +x ban_iptables.sh && bash ban_iptables.sh unbanall
	rm -rf ban_iptables.sh
}
Set_config_connect_verbose_info(){
	SSR_installation_status
...
Update_Shell(){
	sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "https://raw.githubusercontent.com/ichenxin/ssrmu/master/ssrmu.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1) && sh_new_type="github"
	...
	cd "${file}"
	wget -N --no-check-certificate "https://raw.githubusercontent.com/ichenxin/ssrmu/master/ssrmu.sh" && chmod +x ssrmu.sh
